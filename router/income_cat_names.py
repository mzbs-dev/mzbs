...
# def create_income_cat_name( user: Annotated[User, Depends(require_permission("setup_income_categories", "add"))], income_cat_name: IncomeCatNamesCreate, session: Session = Depends(get_session),):
...
# def delete_income_cat_name(user: Annotated[User, Depends(require_permission("setup_income_categories", "delete"))], income_cat_id: int, session: Session = Depends(get_session)):
#     income_cat_name = session.get(IncomeCatNames, income_cat_id)
#     if not income_cat_name:
#         raise HTTPException(
#             status_code=404, detail="Income category name not found")
...
@income_cat_names_router.post("/add_income_cat_name/", response_model=IncomeCatNamesResponse)
def create_income_cat_name(
    user: Annotated[User, Depends(require_permission("setup_income_categories", "add"))],
    income_cat_name: IncomeCatNamesCreate,
    session: Session = Depends(get_session),
    ...
    try:
        session.commit()
        session.refresh(db_income_cat_name)
        cache_invalidate("income_cats", payload.tenant_id)
    # sqlmodel uses SQLAlchemy exceptions for integrity errors
    except IntegrityError as e:
        session.rollback()
        logger.error(f"Integrity error: {e}")
...
def read_income_cat_names(
    ...
    cached = cache_get("income_cats", payload.tenant_id)
    if cached is not None:
        return cached
    income_cat_names = session.exec(select(IncomeCatNames)).all()
    result = [IncomeCatNamesResponse.model_validate(c) for c in income_cat_names]
    cache_set("income_cats", payload.tenant_id, result)
    return result
...
@income_cat_names_router.delete("/del/{income_cat_id}", response_model=dict)
def delete_income_cat_name(
    user: Annotated[User, Depends(require_permission("setup_income_categories", "delete"))],
    income_cat_id: int,
    session: Session = Depends(get_session),
    payload: TokenPayload = Depends(get_token_payload),
):
