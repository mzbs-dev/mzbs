      ...
      <div className="flex flex-col gap-4 mb-6">
        <div className="relative w-full">
          ...
          <Input
            ...
            className="pl-10 pr-4 py-2 w-full rounded-lg border-border focus:border-purple-500 focus:ring focus:ring-secondary/20 transition-all duration-300"
          />
        </div>
      </div>

      {/* Desktop Table View - hidden on mobile */}
      <div className="hidden sm:block rounded-md border border-border overflow-hidden transition-shadow duration-300 hover:shadow-md">
        <Table>
            ...
            {table.getHeaderGroups().map((headerGroup) => (
              <TableRow key={headerGroup.id}>
                {headerGroup.headers.map((header) => (
                  <TableHead
                    key={header.id}
                    className="font-bold text-white dark:text-foreground px-2 sm:px-4 py-2 sm:py-3 text-xs sm:text-sm"
                  >
      ...
      <div className="sm:hidden space-y-3">
        {loading ? (
          ...
          table.getRowModel().rows.map((row) => (
            <div
              key={row.id}
              className="bg-card dark:bg-card border border-border dark:border-border rounded-lg p-3 space-y-2"
            >
              <div>
                <p className="text-xs font-medium text-muted-foreground dark:text-muted-foreground uppercase">Time Slot</p>
                <p className="text-sm font-semibold text-foreground dark:text-foreground">{row.original.attendance_time}</p>
              </div>
              <div>
